...
businesses.forEach(business => {
  outEl.innerHTML += `<h2>${business.purchasingAgent.nameFirst} ${business.purchasingAgent.nameLast}</h2>`
  outEl.innerHTML += `<h3>${business.companyName}</h3>`
  outEl.innerHTML += `<h3>${business.phoneWork}</h3>`
  outEl.innerHTML += "<hr/>";
})


// Lightning Exercise: Instead of just returning the purchasing agent object, return a new object that has the full name of the 
// purchasing agent, the company name, and the phone number. The data structure is shown below. Use that new data structure to display 
// the agent with their company and phone number